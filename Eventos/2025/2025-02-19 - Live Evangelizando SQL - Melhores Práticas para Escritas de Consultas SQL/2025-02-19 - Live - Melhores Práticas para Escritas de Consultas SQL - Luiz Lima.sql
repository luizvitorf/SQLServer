--------------------------------------------------------------------------------------------------
-- LIVE EVANGELIZANDO SQL - MELHORES PRÁTICAS - CONSULTAS SQL
--------------------------------------------------------------------------------------------------

/*
DICA 01: Utilize padrões para nomenclaturas
DICA 02: Case Sensitive / Accent Sensitive
DICA 03: Faça Identação no seu código!
DICA 04: Versionamento de código
DICA 05: VIEW COM SELECT * FROM
DICA 06: COUNT(*) x COUNT(coluna)
DICA 07: CUIDADOS COM NULL x NOT IN
DICA 08: Criação de Índice – Warning! The maximum key length is X bytes
...
-- OBS: Se a empresa ou cliente já utilizam algum padrão, respeite isso e siga!
...
-- PascalCase -> combina palavras colocando TODAS com a primeira letra maiúscula
...
SELECT * FROM testeCS_AS WHERE nome = 'Luiz Vítor'
...
SELECT * FROM testeCI_AI WHERE nome = 'Luiz Vítor'
...
-- ABRIR A PROCEDURE SP_WHOISACTIVE E VALIDAR A IDENTAÇÃO
...
-- ABRIR O CÓDIGO DA PROCEDURE ABAIXO E VALIDAR O HISTÓRICO DE ALTERAÇÕES:
...
-- Ué, mas a tabela tem 3 colunas Luiz...
select * from TB_TESTE
go
...
-- Resultado da view após a atualização
...
-- IMPORTANTE: EVITE UTILIZAR "SELECT * FROM", AO INVÉS DISSO, ESPECIFIQUE SEMPRE AS COLUNAS QUE DEVEM SER RETORNADAS
...
-- COUNT(*) - conta os valores NULL também!
...
-- COUNT(Idade) - NÃO conta os valores NULL!
...
/*
valor NOT IN (X,Y,Z) é o mesmo que:

valor != X AND valor != Y AND valor != Z

valor != NULL, por padrão, é FALSE, pois o valor de NULL é "desconhecido"

Logo, se um for falso, tudo será falso e não irá retornar nada!!!
*/

-- Quando tiver valores NULL, utilize outras opções!
...
-- Criando a procedure também na base Traces
USE Traces
